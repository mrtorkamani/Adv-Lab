...
Md1 = drift(0) # bis Q2
MQ2x = quadrupole(kq, 0.074, "x")
MQ2z = quadrupole(kq, 0.074, "z")
Md2 = drift(.03+.0785+.316+.02)



tmpx = np.matmul(MQ2x, Md1)
MS2x = np.matmul(Md2, tmpx)

tmpz = np.matmul(MQ2z, Md1)
MS2z = np.matmul(Md2, tmpz)


#print(MS2z)

Md3 = drift(.0785+.185+.04+.06+.074+.03+.0785+.316+.04+.06+.074+.03+.0785+.316+.04+.06)
...
term1x = sp.expand(MS2x[0,0]*MS2x[0,0]) * be  # eps_beta0
term2x = sp.expand(-2*MS2x[0,0]*MS2x[0,1]) * al # eps_alpha0
term3x = sp.expand(MS2x[0,1]**2) * ga        # eps_gamma0    

term1z = sp.expand(MS2z[0,0]*MS2z[0,0]) * be  # eps_beta0
term2z = sp.expand(-2*MS2z[0,0]*MS2z[0,1]) * al # eps_alpha0
term3z = sp.expand(MS2z[0,1]**2) * ga        # eps_gamma0    
...
pop2x = np.array([5.72259098e-03, -4.13962001e-01,  1.07968811e+01])
pop2z = np.array([3.75937815e-03, 2.48335025e-01, 5.05186624e+00])
perr2x= np.array([0.00038831, 0.0093253,  0.3395273])
perr2z= np.array([0.000345,   0.00828507, 0.30165302])
